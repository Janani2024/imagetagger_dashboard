...
def check_fs_root_config(app_configs, **kwargs) -> List[CheckMessage]:
    try:
        _create_check_file(filesystem.root(), settings.IMAGE_PATH + '/check.tmp.jpeg')
        _create_check_file(filesystem.root(), settings.TOOLS_PATH + '/check.tmp.py')
        return []
...
def check_tmp_fs_config(app_configs, **kwargs) -> List[CheckMessage]:
    try:
        _create_check_file(filesystem.tmp(), settings.TMP_IMAGE_PATH + '/check.tmp.jpeg')
        return []
    except Exception as e:
        return [Error(
            'Persistent filesystem is incorrectly configured. Could not create and delete a temporary check file',
